...
public class GameWraperImpl implements GameWraper {
  ...
  private boolean innerTest() {
    ...
    Warrior warrior = createContextResult.getResult().createWarrior("testSession1", Viking.class);
...
//
  ...
  public Result createGame(String ownerUserName
          ...
          , String gameName
          , boolean hidden) {
    Player player = core.findPlayer(ownerUserName);
    if (player == null) {
      Result result = ResultImpl.fail(USER_NOT_LOGGED_IN.getError(ownerUserName));
      core.fireEvent(new EventImpl(null, null, EventType.GAME_CONTEXT_CREATE, new EventDataContainer(result, new String[]{gameName, ownerUserName}), null));
      return result;
    } else {

      return core.createGameContext(player, gameRules
